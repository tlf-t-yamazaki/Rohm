...
STP_RETRY:
            ...
            If (0 <> gSysPrm.stDEV.giTheta) AndAlso (0 <> giClampLessStage) Then 'V5.0.0.9⑧
                'V6.0.5.0⑤↓
                'V4.12.2.0⑤                ROUND4(gdCorrectTheta + gdClampLessTheta)               '5.0.1.0⑨
                'V4.12.2.0⑤                r = SMOVE2(gfBordTableOutPosX(Idx) + gfCorrectPosX - gdClampLessOffsetX,
                'V4.12.2.0⑤                           gfBordTableOutPosY(Idx) + gfCorrectPosY - gdClampLessOffsetY) '#5.0.1.0⑧
                'V6.1.2.0④↓
                If giClampLessOutPos = 0 Then
                    ROUND4(gdClampLessTheta)                                    'V4.12.2.0⑤ 立山科学殿仕様によりクランプレス時の
                    r = SMOVE2(gfBordTableInPosX(Idx), gfBordTableInPosY(Idx))  'V4.12.2.0⑤ 排出位置を供給位置と同じにする
                Else
            ...
            Else
                ...
                If (0 <> gSysPrm.stDEV.giTheta) AndAlso (gSysPrm.stTMN.gsKeimei = MACHINE_TYPE_SL436) Then
                    ...
                    'V6.0.5.0⑤'                    ROUND4(gdCorrectTheta + gdClampLessTheta)
                    ROUND4(gdClampLessTheta)       'V6.0.5.0⑤
                End If
                'V6.0.2.0⑥↑
...
#Region "FrmReset実行サブルーチン"
    ...
    Public Function Sub_CallFrmRset(ByVal ObjSys As SystemNET, ByVal gMode As Integer) As Integer   'V6.0.0.0⑱
        ...
        Try
            ...
            'V4.12.2.4②            If (gMode = cGMODE_ORG) Or (gMode = cGMODE_LDR_ORG) Then
            Select Case (gMode)
...
#Region "NG排出トレイに乗せた数をカウントし、設定値を超えていれば画面表示する"
    ...
    Public Function JudgeNGtrayCount(ByVal Idx As Integer) As Integer
        ...
        If ((giNgBoxCounter + giBreakCounter + giTwoTakeCounter) >= giNgBoxCount(Idx)) Then ' NG排出BOXの収納枚数カウンター > NG排出BOXの収納枚数 ? ###181 2013.01.24
            ...
            ''"ＮＧ排出ボックス満杯","ＮＧ排出ボックスからＮＧ基板を取り除いてから","STARTキーを押すか、OKボタンを押して下さい。"V6.1.1.0⑩
            ' "ＮＧ排出ボックス満杯","ＮＧ排出ボックスからＮＧ基板を取り除いて","STARTキーを押すか、OKボタンを押して下さい。"    V6.1.1.0⑩
            r = Sub_CallFrmMsgDisp(Form1.System1, cGMODE_MSG_DSP, cFRS_ERR_START, True, _
